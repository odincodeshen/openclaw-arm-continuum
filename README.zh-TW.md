...
`/mem list` / `done` / `rm` / `edit` / `snooze` / `digest` / `archive-stale`
完整指令、`due:` / `tag:` metadata 語法、以及 `/rag` 共用的 `tag:<字>` /
`since:`/`before:` 範圍過濾，見 `docs/TRACKER_MEMORY.md`；`/rag #<分類>`
見 `docs/CATEGORY_RAG.md`。
